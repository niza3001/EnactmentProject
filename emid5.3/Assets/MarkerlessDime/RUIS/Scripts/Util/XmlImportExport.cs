...
public class XmlImportExport {
	public static bool ImportInputManager(RUISInputManager inputManager, string filename, TextAsset xmlSchema)
	{
		...
		*/
        ...
        XmlNode optitrackNode = xmlDoc.GetElementsByTagName("OptitrackSettings").Item(0);
        inputManager.enableOptitrack = bool.Parse(optitrackNode.SelectSingleNode("enabled").Attributes["value"].Value);
        inputManager.OptitrackIP = optitrackNode.SelectSingleNode("ip").Attributes["value"].Value;
        inputManager.OptitrackPort = int.Parse(optitrackNode.SelectSingleNode("port").Attributes["value"].Value);
        inputManager.StoryTheme = optitrackNode.SelectSingleNode("storyTheme").Attributes["value"].Value;

        XmlNode rbdNode = xmlDoc.GetElementsByTagName("RigidBodySettings").Item(0);
        for (int i = 0; i <= 15; i++)
        {
                inputManager.rbdXmlScale.Add(new Vector3(float.Parse(rbdNode.SelectSingleNode("RigidBodyId" + i.ToString()).Attributes["value"].Value),
                                            float.Parse(rbdNode.SelectSingleNode("RigidBodyId" + i.ToString()).Attributes["value"].Value),
                                            float.Parse(rbdNode.SelectSingleNode("RigidBodyId" + i.ToString()).Attributes["value"].Value)));
...
//		string magnetometerMode = riftDriftNode.SelectSingleNode("magnetometerDriftCorrection").Attributes["value"].Value;
    ...
    public static bool ExportInputManager(RUISInputManager inputManager, string filename) {
        ...
        XmlElement inputManagerRootElement = xmlDoc.CreateElement("ns2", "DIMEInputManager", "http://teilab.tamu.edu/index.php?title=TAMU_Embodied_Interaction_Lab_%28TEILab%29");
        ...
        XmlElement optitrackSettingsElement = xmlDoc.CreateElement("OptitrackSettings");
        inputManagerRootElement.AppendChild(optitrackSettingsElement);

        XmlElement optitrackEnabledElement = xmlDoc.CreateElement("enabled");
        optitrackEnabledElement.SetAttribute("value", inputManager.enableOptitrack.ToString().ToLowerInvariant());
        optitrackSettingsElement.AppendChild(optitrackEnabledElement);

        XmlElement optitrackIPElement = xmlDoc.CreateElement("ip");
        optitrackIPElement.SetAttribute("value", inputManager.OptitrackIP.ToString());
        optitrackSettingsElement.AppendChild(optitrackIPElement);

        XmlElement optitrackPortElement = xmlDoc.CreateElement("port");
        optitrackPortElement.SetAttribute("value", inputManager.OptitrackPort.ToString());
        optitrackSettingsElement.AppendChild(optitrackPortElement);

        XmlElement optitrackStoryElement = xmlDoc.CreateElement("storyTheme");
        optitrackStoryElement.SetAttribute("value", inputManager.StoryTheme.ToString());
        optitrackSettingsElement.AppendChild(optitrackStoryElement);
        ...
        XmlElement rigidBodySettingsElement = xmlDoc.CreateElement("RigidBodySettings");
        inputManagerRootElement.AppendChild(rigidBodySettingsElement);

        for (int i = 0; i <= 15; i++)
        {
            XmlElement rbdScale = xmlDoc.CreateElement("RigidBodyId" + i.ToString());
            rbdScale.SetAttribute("value", 1.00001f.ToString());
            rigidBodySettingsElement.AppendChild(rbdScale);
        }
	...
	public static bool ExportDisplay(RUISDisplay display, string xmlFilename)
	{
		...
		XmlElement displayRootElement = xmlDoc.CreateElement("ns2", "ruisDisplay", "http://teilab.tamu.edu/index.php?title=TAMU_Embodied_Interaction_Lab_%28TEILab%29");
