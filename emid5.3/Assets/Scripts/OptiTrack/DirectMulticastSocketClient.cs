...
namespace OptitrackManagement
{
	...
	public static class DirectMulticastSocketClient {
        ...
        //nz 3/3/18
        //private static DataStream = null;
		...
		private static void ReadRigidBody(Byte[] b, ref int offset, OptiTrackRigidBody rb)
		{
			try
			{
				...
				Buffer.BlockCopy(b, offset, iData, 0, 4); offset += 4;
				int nMarkers = iData[0]; // We believe that the number of markers corresponds to the ID of an object. O.O. 2/25/2018
                // O.O. 2/25
                //Debug.LogError("Number of markers:");
                //Debug.LogError(nMarkers);
		...
		{
            // OO* 1/29/2018
            Debug.LogError("Start() func was called from DirectMulticastSocketClient.cs .");
            StartClient();
		}
